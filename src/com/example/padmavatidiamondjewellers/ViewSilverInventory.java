...
public class ViewSilverInventory extends Activity {
	DBHelper mydbhelper;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_view_silver_inventory);
		Intent in = getIntent();
		
		ActionBar actionBar=this.getActionBar();
		actionBar.setIcon(R.drawable.pdj);
		actionBar.setTitle("SILVER INVENTORY");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case R.id.action_add:
        	...
        	Intent i=new Intent(this,SilverInventory.class);
    		startActivity(i);
            return true;
		...
		private void view_buy() {
			...
			mydbhelper=new DBHelper(ViewSilverInventory.this, Database.PDJ_DATABASE_NAME, null, Database.PDJ_DATABASE_VERSION);
			mydatabase=mydbhelper.getWritableDatabase();
			Cursor c_count= mydatabase.rawQuery("select * from "+Database.S_TABLE_NAME+" where "+Database.S_OPERATION +" = 'Buy'",null);
			...
			int i=0;
			String col1[]=new String[]{Database.S_DATE,Database.S_OPERATION,Database.S_QTY,Database.S_RATE,
					Database.S_TOTAL,Database.S_STATUS,
					Database.S_BOUGHT_FROM,Database.S_AMT_PAID,Database.S_AMT_PENDING};
			...
			l=(ListView)findViewById(R.id.silver_listView);
			l.setAdapter(new GoldInventoryAdapter(ViewSilverInventory.this,op,date,qty,rate,total,status,count,bought_from,amt_paid,amt_pending));
	    ...
	    public boolean onContextItemSelected(MenuItem item){ 
			...
			mydatabase=mydbhelper.getWritableDatabase();
			String cols[]=new String[]{Database.S_STATUS,Database.S_TOTAL,
					Database.S_AMT_PAID,Database.S_AMT_PENDING,Database.S_KEY};
			Cursor c=mydatabase.query(Database.G_TABLE_NAME, cols, null, null, null, null, null);
			int co=c.getCount();
			c.moveToPosition(co-id-1);
			status=c.getString(0);
			g_bill_no=c.getString(4);
	    	...
	    	if(item.getTitle()=="Paid/Unpaid"){  
	    		...
	    		if(status.equals("Paid")){
                	...
                	Toast.makeText(ViewSilverInventory.this,"Not enough Capital",Toast.LENGTH_LONG).show();
                	onBackPressed();

	    		}
	    		ContentValues cs=new ContentValues();
	    		cs.put(Database.S_STATUS,status);
	    		cs.put(Database.S_AMT_PAID,amt_paid);
	    		cs.put(Database.S_AMT_PENDING,amt_pending);
	    		
	    		mydatabase.update(Database.S_TABLE_NAME,cs,Database.S_KEY+"='"+g_bill_no+"'",null);
	    		onCreate(null);
	    		
	            }    
	            else if(item.getTitle()=="Partially Paid"){  
	                if(status.equals("Unpaid") || status.equals("Partially Paid")){
	                	AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(ViewSilverInventory.this);
	            		...
	            		alertDialogBuilder.setPositiveButton("Ok",new DialogInterface.OnClickListener() {
	            	        ...
	            	        public void onClick(DialogInterface arg0, int arg1) {
	                            ...
	                            if(amt_pending1<0){
	                            	Toast.makeText(ViewSilverInventory.this,"Cannot Proceed",Toast.LENGTH_SHORT).show();
	                            }else{
	                        		if(amt_pending1==0){
	                            		statu="Paid";
	                            		amt_paid1=total1;
	                        		...
	                        		ContentValues cs=new ContentValues();
	                	    		cs.put(Database.S_STATUS,statu);
	                	    		cs.put(Database.S_AMT_PAID,amt_paid1);
	                	    		cs.put(Database.S_AMT_PENDING,amt_pending1);
    	    		...
    	    		mydatabase.delete(Database.S_TABLE_NAME,Database.S_KEY+"='"+g_bill_no+"'",null);
