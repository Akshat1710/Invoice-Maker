...
public class SilverInventory extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_silver_inventory);
		Intent i=getIntent();
		...
		cal=(Button)findViewById(R.id.si_cal);
		date=(EditText)findViewById(R.id.si_date);
		date.setEnabled(false);
		
		qty=(EditText)findViewById(R.id.si_qty);
		rate=(EditText)findViewById(R.id.si_rate);
		total=(TextView)findViewById(R.id.si_total);
		total_t=(TextView)findViewById(R.id.si_total_t);
		seller_info=(EditText)findViewById(R.id.seller_s);
		...
		radiogroup = (RadioGroup) findViewById(R.id.payment_s);
		...
		radiogroup.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				if(checkedId == R.id.rb_unpaid_s){
					payment_status="Unpaid";
				}else if(checkedId == R.id.rb_paid_s){
					payment_status="Paid";
				}else{
					partial="";
					payment_status="Partially Paid";
					AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(SilverInventory.this);
	...
	public void si_cancel(View v){
		
		onBackPressed();
	}
	public void si_done(View v){
		mydbhelper=new DBHelper(this, Database.PDJ_DATABASE_NAME, null, Database.PDJ_DATABASE_VERSION);
		mydatabase=mydbhelper.getWritableDatabase();
		
		Cursor c_count= mydatabase.rawQuery("select * from "+Database.S_TABLE_NAME,null);
		int g_bill_no=0;
		...
		cv.put(Database.S_KEY, g_bill_no+"");
		cv.put(Database.S_DATE, date.getText().toString());
		cv.put(Database.S_QTY, qty.getText().toString());
		cv.put(Database.S_RATE, rate.getText().toString());
		cv.put(Database.S_TOTAL, total.getText().toString());
		cv.put(Database.S_STATUS, payment_status);
		cv.put(Database.S_BOUGHT_FROM, seller_info.getText().toString());
		cv.put(Database.S_AMT_PAID,a_paid );
		cv.put(Database.S_AMT_PENDING, a_pending);
		cv.put(Database.S_OPERATION, "Buy");
		mydatabase.insertOrThrow(Database.S_TABLE_NAME, null, cv);
		mydbhelper.close();
