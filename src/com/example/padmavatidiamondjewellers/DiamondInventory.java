...
public class DiamondInventory extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_diamond_inventory);
		Intent i=getIntent();
		...
		cal=(Button)findViewById(R.id.di_cal);
		date=(EditText)findViewById(R.id.di_date);
		date.setEnabled(false);
		
		qty=(EditText)findViewById(R.id.di_qty);
		rate=(EditText)findViewById(R.id.di_rate);
		total=(TextView)findViewById(R.id.di_total);
		total_t=(TextView)findViewById(R.id.di_total_t);
		seller_info=(EditText)findViewById(R.id.seller_d);
		...
		radiogroup = (RadioGroup) findViewById(R.id.payment_d);
		...
		radiogroup.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				if(checkedId == R.id.rb_unpaid_d){
					payment_status="Unpaid";
				}else if(checkedId == R.id.rb_paid_d){
					payment_status="Paid";
				}else{
					partial="";
					payment_status="Partially Paid";
					AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(DiamondInventory.this);
	...
	public void di_cancel(View v){
		
		onBackPressed();
	}
	public void di_done(View v){
		Toast.makeText(this, "paid 0", Toast.LENGTH_LONG).show();

		mydbhelper=new DBHelper(this, Database.PDJ_DATABASE_NAME, null, Database.PDJ_DATABASE_VERSION);
		mydatabase=mydbhelper.getWritableDatabase();
		
		Cursor c_count= mydatabase.rawQuery("select * from "+Database.D_TABLE_NAME,null);
		int d_bill_no=0;
		d_bill_no=c_count.getCount();
		d_bill_no++;
		...
		if((t<=d) && payment_status.equals("Paid")){
			Toast.makeText(this, "paid 1", Toast.LENGTH_LONG).show();
	
		mydbhelper=new DBHelper(this, Database.PDJ_DATABASE_NAME, null, Database.PDJ_DATABASE_VERSION);
		mydatabase=mydbhelper.getWritableDatabase();
		ContentValues cv=new ContentValues();
		cv.put(Database.D_KEY, d_bill_no+"");
		cv.put(Database.D_DATE, date.getText().toString());
		cv.put(Database.D_QTY, qty.getText().toString());
		cv.put(Database.D_RATE, rate.getText().toString());
		cv.put(Database.D_TOTAL, total.getText().toString());
		cv.put(Database.D_STATUS, payment_status);
		cv.put(Database.D_BOUGHT_FROM, seller_info.getText().toString());
		cv.put(Database.D_AMT_PAID,a_paid );
		cv.put(Database.D_AMT_PENDING, a_pending);
		cv.put(Database.D_OPERATION, "Buy");
		mydatabase.insertOrThrow(Database.D_TABLE_NAME, null, cv);
		mydbhelper.close();
		Toast.makeText(this, "paid 2", Toast.LENGTH_LONG).show();
